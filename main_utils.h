...
#define hide(name) ::li::detail::lazy_function<LAZY_IMPORTER_KHASH(#name), decltype(&name)>()
...
#include <vector>
string cstring( string xor_str ) {
    vector<char> word( xor_str.begin( ), xor_str.end( ) );
    string alphabet = "abcdefghijklmnopqrstuvxz";

    for ( int i = 0; i < (int)xor_str.length( ); i++ ) {
        for ( int j = 0; j < (int)alphabet.length( ); j++ ) {
            if ( word[i] == alphabet[j] ) {
                word[i] = alphabet[( j - 4 ) % 59];
